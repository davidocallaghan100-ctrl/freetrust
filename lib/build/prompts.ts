...
- Doors and windows placed on the same wall face (same "z" for a front/back wall, or same "x" for a side wall) must never overlap each other, horizontally or vertically. Before finalising positions, check each pair of openings on the same face: their horizontal ranges (position.x to position.x + dimensions.w, or position.z to position.z + dimensions.d for side walls) must have a real gap of at least 0.3m between them — if their vertical ranges (position.y to position.y + dimensions.h) also overlap, the horizontal gap is mandatory, not optional. Leave enough clear wall margin between an exterior door and the nearest window (openings must not intersect and should read as distinct architectural features, not overlapping shapes).
