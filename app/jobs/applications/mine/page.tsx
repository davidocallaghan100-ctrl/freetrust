...
const STATUS_CONFIG: Record<AppStatus, { label: string; emoji: string; color: string; bg: string; border: string; message?: string }> = {
  ...
  reviewed:    { label: 'Reviewed',    emoji: '👀', color: 'var(--ft-accent)', bg: 'rgba(56,189,248,0.1)',   border: 'rgba(56,189,248,0.3)' },
  ...
  rejected:    { label: 'Rejected',    emoji: '❌', color: 'var(--ft-danger)', bg: 'rgba(248,113,113,0.1)',  border: 'rgba(248,113,113,0.3)', message: 'Keep going — the right opportunity is out there.' },
}
...
function ApplicationCard({ app }: { app: JobApplication }) {
  ...
  const jobTypeColor = job?.job_type ? JOB_TYPE_COLORS[job.job_type] ?? 'var(--ft-text-tertiary)' : 'var(--ft-text-tertiary)'
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '0.9rem', alignItems: 'flex-start' }}>
        ...
        <div style={{
          ...
          background: 'var(--ft-bg)', border: '1px solid rgba(148,163,184,0.15)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '1.3rem', fontWeight: 800, color: 'var(--ft-accent)',
        }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem', flexWrap: 'wrap' }}>
            <div>
              {job
                ? <Link href={`/jobs/${job.id}`} style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--ft-text)', textDecoration: 'none' }}>{job.title}</Link>
                : <span style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--ft-text-tertiary)' }}>Job no longer available</span>
              }
              {job?.company_name && (
                <div style={{ fontSize: '0.8rem', color: 'var(--ft-text-tertiary)', marginTop: 2 }}>{job.company_name}</div>
              )}
            </div>
            {/* Status badge */}
            <span style={{
          ...
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginTop: '0.45rem' }}>
            ...
            <span style={{ color: 'var(--ft-text-faint)', fontSize: '0.72rem' }}>Applied {daysAgo(app.created_at)}</span>
          </div>
        </div>
      </div>
      ...
      {app.cover_letter && (
        <div style={{ fontSize: '0.83rem', color: 'var(--ft-text-secondary)', lineHeight: 1.5 }}>
          {expanded ? app.cover_letter : preview}
          {hasMore && (
            <button onClick={() => setExpanded(e => !e)} style={{
              background: 'none', border: 'none', color: 'var(--ft-accent)', cursor: 'pointer',
      ...
      {(app.cv_url || app.portfolio_url) && (
        <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
          {app.cv_url && (
            <a href={app.cv_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.78rem', color: 'var(--ft-accent)', textDecoration: 'none' }}>
              📄 CV / Resume
            </a>
          )}
          {app.portfolio_url && (
            <a href={app.portfolio_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.78rem', color: 'var(--ft-accent)', textDecoration: 'none' }}>
              🔗 Portfolio
            </a>
          )}
        </div>
      ...
      {job && (
        <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', paddingTop: '0.25rem', borderTop: '1px solid rgba(148,163,184,0.08)' }}>
          <Link
            ...
            style={{
              background: 'rgba(56,189,248,0.1)', color: 'var(--ft-accent)',
          ...
          <Link
            href={`/jobs/${job.id}`}
            style={{
              background: 'transparent', color: 'var(--ft-text-secondary)',
  ...
  if (authed === null || (authed && loading)) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ color: 'var(--ft-accent)', fontSize: '1rem' }}>Loading your applications…</div>
      </div>
    )
  }

  return (
    <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', color: 'var(--ft-text)', paddingBottom: '4rem' }}>
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '1.5rem 1rem' }}>

        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div>
            <Link href="/jobs" style={{ color: 'var(--ft-text-tertiary)', fontSize: '0.82rem', textDecoration: 'none', display: 'inline-block', marginBottom: '0.5rem' }}>
              ← Back to Jobs
            </Link>
            <h1 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0 }}>My Applications</h1>
            <p style={{ color: 'var(--ft-text-tertiary)', margin: '0.25rem 0 0', fontSize: '0.9rem' }}>Track the status of your job applications</p>
          </div>
          <Link href="/jobs" style={{
            background: 'var(--ft-accent)', color: 'var(--ft-bg)', borderRadius: 8,
        ...
        {stats.total > 0 && (
          <div style={{
            ...
            {[
              { label: 'Total', value: stats.total, color: 'var(--ft-text)' },
              { label: 'Pending', value: stats.pending, color: '#fbbf24' },
              { label: 'Shortlisted', value: stats.shortlisted, color: '#a78bfa' },
              { label: 'Hired', value: stats.hired, color: '#34d399' },
            ].map(s => (
              <div key={s.label} style={{
                background: 'var(--ft-surface)', borderRadius: 10, padding: '0.85rem',
                border: '1px solid rgba(148,163,184,0.1)', textAlign: 'center',
              }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, color: s.color }}>{s.value}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--ft-text-tertiary)', marginTop: 2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        )}
          ...
          {TABS.map(tab => (
            <button
              ...
              style={{
                ...
                color: filter === tab.key ? 'var(--ft-accent)' : 'var(--ft-text-tertiary)',
        ...
        {filtered.length === 0 ? (
          <div style={{
            background: 'var(--ft-surface)', borderRadius: 12, padding: '3rem 2rem',
            ...
            {stats.total === 0 ? (
              <>
                ...
                <div style={{ color: 'var(--ft-text-tertiary)', fontSize: '0.88rem', marginBottom: '1.25rem' }}>
              ...
              <>
                ...
                <div style={{ color: 'var(--ft-text-tertiary)', fontSize: '0.9rem' }}>No {filter} applications.</div>
              </>
            )}
          </div>
        ) : (
