...
export default function ArticlePage() {
  ...
  if (notFound || !article) return (
    <div style={{ minHeight: 'calc(100vh - 58px)', background: 'var(--ft-bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
      <div style={{ fontSize: '3rem' }}>📄</div>
      <h1 style={{ color: 'var(--ft-text)', fontWeight: 800 }}>Article not found</h1>
      <Link href="/articles" style={{ color: 'var(--ft-accent)', textDecoration: 'none', fontSize: '0.9rem' }}>← Back to Articles</Link>
    </div>
  )

  const authorInitials = getInitials(article.author?.full_name)
  const isAuthor = userId === article.author_id
  const catColor = CAT_COLOR[article.category ?? ''] ?? 'var(--ft-accent)'
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 58px)', background: 'var(--ft-bg)', color: 'var(--ft-text)', fontFamily: 'system-ui' }}>
      <style>{`
        ...
        .art-body a { color: var(--ft-accent); }
        .art-body p { margin: 0.75rem 0; }
        .clap-btn { background: var(--ft-surface); border: 2px solid rgba(56,189,248,0.2); border-radius: 50%; width: 56px; height: 56px; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; transition: all 0.15s; font-size: 1.4rem; position: relative; }
      ...
      <div style={{ position: 'fixed', top: 58, left: 0, right: 0, height: 3, background: 'rgba(56,189,248,0.12)', zIndex: 999 }}>
        <div style={{ height: '100%', background: 'var(--ft-accent)', width: `${scrollProgress}%`, transition: 'width 0.1s linear' }} />
      </div>
        ...
        <div ref={articleContentRef}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
            ...
            {article.tags.slice(0, 4).map(t => (
              <span key={t} style={{ borderRadius: 999, padding: '0.15rem 0.55rem', fontSize: '0.72rem', background: 'rgba(56,189,248,0.06)', border: '1px solid rgba(56,189,248,0.15)', color: 'var(--ft-accent)' }}>#{t}</span>
            ))}
            {isAuthor && (
              <span style={{ background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.3)', borderRadius: 999, padding: '0.2rem 0.7rem', fontSize: '0.72rem', fontWeight: 700, color: 'var(--ft-accent)' }}>₮20 earned for publishing</span>
            )}
          </div>

          {/* Title */}
          <h1 style={{ fontSize: '2.1rem', fontWeight: 900, lineHeight: 1.2, marginBottom: '1rem', color: 'var(--ft-text)' }}>{article.title}</h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.75rem', paddingBottom: '1.5rem', borderBottom: '1px solid rgba(56,189,248,0.08)' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.85rem', color: 'var(--ft-bg)', background: getAvatarGrad(authorInitials), flexShrink: 0 }}>{authorInitials}</div>
            <div>
          ...
          {/* Excerpt */}
          {article.excerpt && (
            <p style={{ fontSize: '1.1rem', color: 'var(--ft-text-secondary)', lineHeight: 1.7, marginBottom: '1.5rem', fontStyle: 'italic', borderLeft: '3px solid rgba(56,189,248,0.3)', paddingLeft: '1rem' }}>{article.excerpt}</p>
          )}
          ...
          <div style={{ marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid rgba(56,189,248,0.08)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--ft-text-tertiary)', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--ft-text)' }}>{clapCount.toLocaleString()} claps</div>
            {userClapCount > 0 && <div style={{ fontSize: '0.78rem', color: 'var(--ft-accent)' }}>You gave {userClapCount} / 50 claps</div>}
          </div>

          {/* Author profile card */}
          <div style={{ marginTop: '3rem', background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.12)', borderRadius: 14, padding: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1rem', color: 'var(--ft-bg)', background: getAvatarGrad(authorInitials), flexShrink: 0 }}>{authorInitials}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--ft-text)', marginBottom: '0.25rem' }}>{article.author?.full_name ?? 'Author'}</div>
              <div style={{ fontSize: '0.8rem', color: 'var(--ft-text-tertiary)', marginBottom: '0.6rem' }}>{authorArticleCount} published articles on FreeTrust</div>
              {article.author?.bio && <p style={{ fontSize: '0.88rem', color: 'var(--ft-text-secondary)', lineHeight: 1.6, marginBottom: '0.75rem' }}>{article.author.bio}</p>}
              <button style={{ background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: 7, padding: '0.4rem 1rem', fontSize: '0.82rem', fontWeight: 600, color: 'var(--ft-accent)', cursor: 'pointer' }}>+ Follow</button>
            </div>
          </div>

          {/* Comments */}
          <div style={{ marginTop: '3rem' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '1.25rem', color: 'var(--ft-text)' }}>Comments ({comments.length})</h3>

            {/* Add comment */}
            <div style={{ background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.12)', borderRadius: 10, padding: '1rem', marginBottom: '1.5rem' }}>
              <textarea
                value={commentText}
                ...
                disabled={!userId}
                rows={3}
                style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', color: 'var(--ft-text)', fontSize: '0.9rem', lineHeight: 1.6, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }}
              />
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
                <button
                  onClick={handleComment}
                  disabled={!commentText.trim() || submittingComment || !userId}
                  style={{ background: 'var(--ft-accent)', border: 'none', borderRadius: 7, padding: '0.45rem 1rem', fontSize: '0.85rem', fontWeight: 700, color: 'var(--ft-bg)', cursor: 'pointer', opacity: (!commentText.trim() || submittingComment || !userId) ? 0.5 : 1 }}
                >
            ...
            {/* Comment list */}
            {comments.length === 0
              ? <div style={{ textAlign: 'center', color: 'var(--ft-text-faint)', padding: '2rem', fontSize: '0.9rem' }}>No comments yet — be the first to comment</div>
              : comments.map(c => {
                const cInitials = getInitials(c.author?.full_name)
                return (
                  <div key={c.id} className="comment-card">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', marginBottom: '0.6rem' }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.7rem', color: 'var(--ft-bg)', background: getAvatarGrad(cInitials), flexShrink: 0 }}>{cInitials}</div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: '0.85rem', color: 'var(--ft-text)' }}>{c.author?.full_name ?? 'Member'}</div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--ft-text-faint)' }}>{timeAgo(c.created_at)}</div>
                      </div>
                    </div>
                    <p style={{ fontSize: '0.88rem', color: 'var(--ft-text-secondary)', lineHeight: 1.65, margin: 0 }}>{c.body}</p>
                  </div>
                )
              })}
          </div>
          ...
          {related.length > 0 && (
            <div style={{ marginTop: '3rem', display: 'none' }} className="art-mobile-related">
              <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: 'var(--ft-text)' }}>Related Articles</h3>
              {related.map(r => {
                ...
                return (
                  <Link key={r.id} href={`/articles/${r.slug}`} className="related-card" style={{ marginBottom: '0.75rem' }}>
                    <div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--ft-text)', lineHeight: 1.35, marginBottom: '0.5rem' }}>{r.title}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem', color: 'var(--ft-text-tertiary)' }}>
                      <div style={{ width: 20, height: 20, borderRadius: '50%', background: getAvatarGrad(rInit), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--ft-bg)' }}>{rInit}</div>
        ...
        <aside className="art-read-sidebar">
          ...
          <div style={{ background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.12)', borderRadius: 12, padding: '1.25rem', textAlign: 'center' }}>
            ...
            <button
              ...
              style={{ margin: '0 auto' }}
            >👏</button>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--ft-text)', marginTop: '0.6rem' }}>{clapCount.toLocaleString()}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-tertiary)' }}>claps</div>
            {userClapCount > 0 && <div style={{ fontSize: '0.72rem', color: 'var(--ft-accent)', marginTop: '0.3rem' }}>{userClapCount}/50 given</div>}
          </div>

          {/* Related articles */}
          {related.length > 0 && (
            <div style={{ background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.1)', borderRadius: 12, padding: '1.25rem' }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--ft-text)', marginBottom: '1rem' }}>Related Articles</div>
              {related.map(r => {
                const rInit = getInitials(r.author?.full_name)
                return (
                  <Link key={r.id} href={`/articles/${r.slug}`} className="related-card" style={{ marginBottom: '0.75rem', display: 'block' }}>
                    <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--ft-text)', lineHeight: 1.35, marginBottom: '0.5rem' }}>{r.title}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.72rem', color: 'var(--ft-text-tertiary)' }}>
                      <div style={{ width: 18, height: 18, borderRadius: '50%', background: getAvatarGrad(rInit), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.55rem', fontWeight: 700, color: 'var(--ft-bg)', flexShrink: 0 }}>{rInit}</div>
                      <span>{r.author?.full_name ?? 'Author'}</span>
                      <span>·</span>
                      <span>👏 {r.clap_count}</span>
                    </div>
          ...
          {/* Back to articles */}
          <Link href="/articles" style={{ display: 'block', textAlign: 'center', padding: '0.6rem', borderRadius: 8, border: '1px solid rgba(148,163,184,0.15)', color: 'var(--ft-text-secondary)', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 500 }}>← All Articles</Link>
          ...
          <div style={{ background: 'rgba(56,189,248,0.04)', border: '1px solid rgba(56,189,248,0.15)', borderRadius: 12, padding: '1.1rem', textAlign: 'center' }}>
            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--ft-accent)', marginBottom: '0.4rem' }}>Write for FreeTrust</div>
            <p style={{ fontSize: '0.78rem', color: 'var(--ft-text-tertiary)', lineHeight: 1.5, marginBottom: '0.75rem' }}>Earn ₮20 Trust for every article you publish.</p>
            <Link href="/articles/new" style={{ display: 'block', background: 'var(--ft-accent)', borderRadius: 7, padding: '0.5rem', fontSize: '0.82rem', fontWeight: 700, color: 'var(--ft-bg)', textDecoration: 'none' }}>Start Writing</Link>
          </div>
        </aside>
      </div>
