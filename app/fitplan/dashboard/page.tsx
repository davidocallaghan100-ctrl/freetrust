...
export default function FitPlanDashboardPage() {
  ...
  const progressScore = useMemo(() => { const workouts = plan?.workouts ?? []; const workoutDone = workouts.length ? Object.values(plan?.completedWorkouts ?? {}).filter(Boolean).length / workouts.length : 0; const rows = data?.progress ?? []; const logsDone = rows.length ? rows.filter(r => r.workout_completed || r.nutrition_hit).length / rows.length : 0; return Math.round(Math.max(workoutDone, logsDone) * 100) }, [data, plan])
  ...
  async function toggleWorkout(index: number, completed: boolean) { setBusy(`complete-${index}`); setError(''); const res = await fetch('/api/fitplan/complete-workout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ index, completed }) }); const json = await res.json().catch(() => ({})); setBusy(''); if (!res.ok) { setError(json.error || 'Could not update workout'); return } await load() }
  ...
  return <main style={{ minHeight: '100vh', background: `radial-gradient(circle at top left, rgba(16,185,129,.18), transparent 35%), radial-gradient(circle at 90% 10%, rgba(56,189,248,.16), transparent 28%), ${C.bg}`, color: C.text, padding: '14px 12px 104px' }}>
    <div style={{ maxWidth: 1180, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.25fr) minmax(280px, .75fr)', gap: 12, marginTop: 12 }} className="fitplan-grid">
        <Card><h2 style={{ marginTop: 0 }}>7-day plan</h2>{plan ? <div style={{ display: 'grid', gap: 12 }}>{(plan.workouts ?? []).map((w: any, i: number) => { const done = Boolean(completedWorkouts[String(i)]); return <div key={`${w.day}-${i}`} style={{ borderRadius: 20, background: done ? 'rgba(16,185,129,.08)' : C.card2, border: `1px solid ${done ? 'rgba(16,185,129,.34)' : C.line}`, overflow: 'hidden' }}><div style={{ minHeight: 118, backgroundImage: `linear-gradient(180deg, rgba(2,6,23,.12), rgba(2,6,23,.82)), url(${workoutImageFor(w.focus ?? '')})`, backgroundSize: 'cover', backgroundPosition: 'center', padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}><div style={{ display: 'flex', gap: 10, alignItems: 'center' }}><div style={{ width: 38, height: 38, borderRadius: 999, display: 'grid', placeItems: 'center', background: done ? C.green : 'rgba(2,6,23,.72)', border: `1px solid ${done ? C.green : C.line}`, fontWeight: 950 }}>{i + 1}</div><div><strong style={{ display: 'block', fontSize: 18 }}>Day {i + 1} · {w.day}</strong><span style={{ color: C.muted, fontWeight: 800 }}>{w.focus}</span></div></div><Pill color={C.gold}>{w.durationMinutes} min</Pill></div><div style={{ padding: 12 }}><ol style={{ margin: 0, paddingLeft: 22, color: C.muted, lineHeight: 1.55 }}>{numberedBlocks(w.blocks).map((block, blockIndex) => <li key={blockIndex} style={{ marginBottom: 6 }}>{block}</li>)}</ol>{Array.isArray(w.modifications) && w.modifications.length ? <p style={{ color: C.gold, margin: '8px 0 0', fontSize: 13, lineHeight: 1.45 }}>Modify: {w.modifications.join(' · ')}</p> : null}<button onClick={() => toggleWorkout(i, !done)} disabled={busy === `complete-${i}`} style={{ width: '100%', marginTop: 12, minHeight: 44, border: 'none', borderRadius: 14, background: done ? 'rgba(16,185,129,.18)' : C.green, color: done ? C.green : '#fff', fontWeight: 950 }}>{done ? '✓ Completed — tap to undo' : 'Mark workout complete'}</button></div></div> })}</div> : <p style={{ color: C.muted }}>No active plan yet. Generate a plan to fill this section.</p>}</Card>
        <div style={{ display: 'grid', gap: 12 }}><Card><h2 style={{ marginTop: 0 }}>Nutrition plan</h2>{plan?.nutrition ? <div style={{ display: 'grid', gap: 10 }}><p style={{ color: C.muted, lineHeight: 1.5, marginTop: 0 }}>{plan.nutrition.approach}</p>{(plan.nutrition.meals ?? []).map((meal: string, i: number) => <div key={`${meal}-${i}`} style={{ minHeight: 112, borderRadius: 18, overflow: 'hidden', border: `1px solid ${C.line}`, backgroundImage: `linear-gradient(90deg, rgba(2,6,23,.86), rgba(2,6,23,.22)), url(${mealImageFor(i)})`, backgroundSize: 'cover', backgroundPosition: 'center', padding: 12, display: 'flex', alignItems: 'end' }}><div><Pill color={C.cyan}>Meal {i + 1}</Pill><p style={{ margin: '8px 0 0', fontWeight: 850, lineHeight: 1.4 }}>{meal}</p></div></div>)}{Array.isArray(plan.nutrition.prepTips) && <ol style={{ margin: 0, paddingLeft: 20, color: C.muted, lineHeight: 1.5 }}>{plan.nutrition.prepTips.map((tip: string, i: number) => <li key={i}>{tip}</li>)}</ol>}</div> : <p style={{ color: C.muted }}>Generate your plan to see meal ideas, prep tips, and nutrition guidance.</p>}</Card><Card><h2 style={{ marginTop: 0 }}>Credits</h2><p style={{ color: C.muted }}>Plans and coach messages spend Trust Coins. Check-ins and logs can earn small Trust rewards.</p>{[['starter','₮250','€4.99'],['momentum','₮600','€9.99'],['coach','₮1500','€19.99']].map(([id, trust, price]) => <button key={id} onClick={() => topup(id)} disabled={busy === `topup-${id}`} style={{ width: '100%', marginTop: 8, minHeight: 44, borderRadius: 14, border: `1px solid ${C.line}`, background: 'rgba(255,255,255,.05)', color: C.text, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 12px', fontWeight: 900 }}><span>{trust}</span><span>{price}</span></button>)}</Card></div>
      </div>
