...
function StatCard({ label, value, sub, accent = 'var(--ft-accent)' }: { label: string; value: string; sub?: string; accent?: string }) {
  return (
...
function BarViz({ data, color = 'var(--ft-accent)' }: { data: ChartPoint[]; color?: string }) {
  if (!data.some(point => point.value > 0)) return <EmptyState>No data yet.</EmptyState>
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: -18 }}>
        <CartesianGrid stroke="rgba(148,163,184,0.12)" strokeDasharray="3 3" />
        <XAxis dataKey="label" tick={{ fill: 'var(--ft-text-tertiary)', fontSize: 10 }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
        <YAxis tick={{ fill: 'var(--ft-text-tertiary)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} />
        <Tooltip contentStyle={{ background: 'var(--ft-bg)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: 10, color: '#e2e8f0' }} />
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: -18 }}>
        ...
        <XAxis dataKey="label" tick={{ fill: 'var(--ft-text-tertiary)', fontSize: 10 }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
        <YAxis tick={{ fill: 'var(--ft-text-tertiary)', fontSize: 10 }} tickLine={false} axisLine={false} allowDecimals={false} />
        <Tooltip contentStyle={{ background: 'var(--ft-bg)', border: '1px solid rgba(52,211,153,0.25)', borderRadius: 10, color: '#e2e8f0' }} />
  ...
  return (
    <main className="ft-admin-shell" id="main-content">
      <style>{`
        ...
        .ft-admin-brand{display:flex;align-items:center;gap:.75rem;padding:.6rem .55rem 1.1rem;border-bottom:1px solid rgba(148,163,184,.1);margin-bottom:1rem}.ft-admin-logo{width:38px;height:38px;border-radius:14px;background:linear-gradient(135deg,var(--ft-accent),#00d4aa);display:grid;place-items:center;color:#06111f;font-weight:900}.ft-admin-brand strong{display:block}.ft-admin-brand span{display:block;color:var(--ft-text-tertiary);font-size:.78rem;margin-top:.1rem}
        .ft-admin-nav{display:grid;gap:.35rem}.ft-admin-nav a{display:block;padding:.72rem .78rem;border-radius:13px;color:var(--ft-text-secondary);font-size:.88rem;font-weight:750}.ft-admin-nav a:hover{background:rgba(56,189,248,.1);color:#e0f2fe}.ft-admin-secure{margin-top:1.25rem;padding:.8rem;border-radius:16px;border:1px solid rgba(52,211,153,.25);background:rgba(52,211,153,.07);font-size:.76rem;color:#86efac;line-height:1.5}
        .ft-admin-main{min-width:0;padding:1.25rem 1.4rem 3rem}.ft-admin-topbar{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;margin-bottom:1.2rem;padding:1rem 1.05rem;border:1px solid rgba(56,189,248,.13);border-radius:22px;background:rgba(15,23,42,.72);box-shadow:0 24px 80px rgba(0,0,0,.25)}.ft-admin-topbar h1{font-size:clamp(1.45rem,2.8vw,2.15rem);margin:0;letter-spacing:-.04em}.ft-admin-topbar p{margin:.25rem 0 0;color:var(--ft-text-tertiary);font-size:.88rem}.ft-admin-email{text-align:right;color:var(--ft-text-secondary);font-size:.82rem}.ft-admin-email strong{display:block;color:#e2e8f0;margin-bottom:.2rem}
        .ft-admin-section{scroll-margin-top:1rem;margin-bottom:1.25rem}.ft-admin-section-head{display:flex;align-items:center;justify-content:space-between;gap:1rem;margin:1.4rem 0 .75rem}.ft-admin-section h2{margin:0;font-size:1.05rem}.ft-admin-section p{margin:.15rem 0 0;color:var(--ft-text-tertiary);font-size:.78rem}.ft-admin-refresh{min-height:44px;padding:.55rem .85rem;border-radius:12px;border:1px solid rgba(56,189,248,.22);background:rgba(56,189,248,.08);color:var(--ft-accent);font-weight:800;font-size:.82rem}.ft-admin-refresh:disabled{opacity:.6}
        .ft-admin-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.8rem}.ft-admin-two{display:grid;grid-template-columns:1fr 1fr;gap:.8rem}.ft-admin-three{display:grid;grid-template-columns:1fr 1fr 1fr;gap:.8rem}.ft-admin-card{background:linear-gradient(180deg,rgba(15,23,42,.92),rgba(15,23,42,.72));border:1px solid rgba(148,163,184,.14);border-radius:20px;padding:1rem;box-shadow:0 20px 70px rgba(0,0,0,.18);min-width:0}.ft-admin-card h3{font-size:.88rem;margin:0 0 .8rem;color:#f8fafc}.ft-admin-stat-value{font-size:1.75rem;font-weight:900;letter-spacing:-.04em;line-height:1}.ft-admin-stat-label{margin-top:.45rem;color:var(--ft-text-secondary);text-transform:uppercase;letter-spacing:.07em;font-size:.68rem;font-weight:900}.ft-admin-stat-sub{color:var(--ft-text-tertiary);font-size:.76rem;margin-top:.2rem}
        .ft-admin-chart-card{min-height:290px}.ft-admin-empty{border:1px dashed rgba(148,163,184,.22);border-radius:15px;color:var(--ft-text-tertiary);padding:1.2rem;font-size:.82rem;line-height:1.55;background:rgba(2,6,23,.22)}.ft-admin-note{color:var(--ft-text-secondary);font-size:.82rem;line-height:1.6}.ft-admin-note code,.ft-admin-code{font-family:var(--font-geist-mono),monospace;font-size:.75rem;color:#bae6fd;white-space:pre-wrap;word-break:break-word}.ft-admin-status{display:inline-flex;align-items:center;gap:.35rem;border:1px solid rgba(52,211,153,.22);background:rgba(52,211,153,.08);color:#86efac;border-radius:999px;padding:.28rem .55rem;font-size:.74rem;font-weight:800}.ft-admin-warn{border-color:rgba(251,191,36,.22);background:rgba(251,191,36,.08);color:#fde68a}
        .ft-admin-count-list{display:grid;gap:.72rem}.ft-admin-count-row{display:grid;grid-template-columns:minmax(88px,1.2fr) minmax(80px,2fr) auto;gap:.65rem;align-items:center;font-size:.8rem;color:var(--ft-text-secondary)}.ft-admin-count-row div{height:8px;border-radius:999px;background:rgba(148,163,184,.13);overflow:hidden}.ft-admin-count-row i{display:block;height:100%;border-radius:999px;background:linear-gradient(90deg,var(--ft-accent),#00d4aa)}.ft-admin-count-row strong{color:#f8fafc}.ft-admin-table-wrap{overflow:auto}.ft-admin-table{width:100%;border-collapse:collapse;min-width:540px}.ft-admin-table th{text-align:left;color:var(--ft-text-tertiary);font-size:.68rem;text-transform:uppercase;letter-spacing:.08em;padding:.65rem;border-bottom:1px solid rgba(148,163,184,.12)}.ft-admin-table td{padding:.7rem .65rem;border-bottom:1px solid rgba(148,163,184,.08);font-size:.8rem;color:var(--ft-text-secondary)}.ft-admin-table tr:last-child td{border-bottom:0}.ft-admin-pill{display:inline-block;padding:.2rem .48rem;border-radius:999px;background:rgba(56,189,248,.1);color:var(--ft-accent);font-size:.72rem;font-weight:800}
          ...
          <>
            ...
            <SectionCard id="trust" title="Trust Score Analytics" updatedAt={updatedAt} onRefresh={loadData} loading={loading}>
              <div className="ft-admin-grid">
                ...
                <StatCard label="Score source" value={data.trustScores.available ? 'Live' : 'Not found'} sub="No fake inferred score" accent="var(--ft-danger)" />
            ...
            <SectionCard id="email" title="Email Notifications Analytics" updatedAt={updatedAt} onRefresh={loadData} loading={loading}>
              <div className="ft-admin-grid">
                ...
                <StatCard label="Email opt-out disabled" value={fmt(data.emailNotifications.optInDisabled)} accent="var(--ft-danger)" />
            ...
            <SectionCard id="health" title="Platform Health" updatedAt={updatedAt} onRefresh={loadData} loading={loading}>
              <div className="ft-admin-grid">
                ...
                <StatCard label="API query errors" value={fmt(data.dataSource.errors.length)} accent={data.dataSource.errors.length ? 'var(--ft-danger)' : '#34d399'} />
