...
function CheckoutContent() {
  ...
  if (status === 'loading') {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 36, height: 36, border: '3px solid rgba(56,189,248,0.2)', borderTopColor: 'var(--ft-accent)', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
  ...
  if (status === 'error' || !service) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', color: 'var(--ft-text)', padding: '2rem', textAlign: 'center' }}>
        <div style={{ fontSize: '3rem' }}>⚠️</div>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 800, margin: 0 }}>Service unavailable</h2>
        <p style={{ color: 'var(--ft-text-tertiary)', margin: 0, maxWidth: 400 }}>{error || 'This service could not be loaded.'}</p>
        <Link href="/services" style={{ marginTop: '0.5rem', background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.3)', borderRadius: 10, padding: '0.6rem 1.5rem', color: 'var(--ft-accent)', fontWeight: 700, textDecoration: 'none', fontSize: '0.9rem' }}>
          ← Browse Services
        </Link>
      </div>
    )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', color: 'var(--ft-text)', fontFamily: 'system-ui', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      <div style={{ width: '100%', maxWidth: 480 }}>

        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'linear-gradient(135deg,var(--ft-accent),#818cf8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', margin: '0 auto 1rem' }}>
            🛠
          </div>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 900, margin: '0 0 0.4rem', letterSpacing: '-0.5px' }}>Confirm Booking</h1>
          <p style={{ color: 'var(--ft-text-tertiary)', margin: 0, fontSize: '0.9rem' }}>Review your order before proceeding to payment</p>
        </div>

        {/* Order card */}
        <div style={{ background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.15)', borderRadius: 16, padding: '1.5rem', marginBottom: '1rem' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--ft-text-faint)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>Service</div>
          <div style={{ fontWeight: 800, fontSize: '1.05rem', color: 'var(--ft-text)', marginBottom: '0.35rem' }}>{service.title}</div>
          <div style={{ fontSize: '0.82rem', color: 'var(--ft-text-tertiary)', marginBottom: '0.75rem', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as React.CSSProperties}>
            {service.description ?? 'Professional service delivered with care.'}
          </div>
          <div style={{ fontSize: '0.8rem', color: 'var(--ft-text-secondary)' }}>by <strong style={{ color: 'var(--ft-text)' }}>{sellerName}</strong>
        ...
        {/* Price breakdown */}
        <div style={{ background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.15)', borderRadius: 16, padding: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          {[
            ...
            <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.45rem 0', borderBottom: '1px solid rgba(56,189,248,0.06)' }}>
              <span style={{ fontSize: '0.88rem', color: row.muted ? 'var(--ft-text-tertiary)' : 'var(--ft-text)' }}>{row.label}</span>
              <span style={{ fontSize: '0.88rem', fontWeight: row.muted ? 400 : 700, color: row.muted ? 'var(--ft-text-tertiary)' : 'var(--ft-accent)' }}>{row.value}</span>
            </div>
          ))}
        ...
        <button
          ...
          style={{ width: '100%', padding: '1rem', background: status === 'paying' ? 'rgba(56,189,248,0.4)' : 'linear-gradient(135deg,var(--ft-accent),#818cf8)', border: 'none', borderRadius: 12, fontSize: '1rem', fontWeight: 800, color: 'var(--ft-bg)', cursor: status === 'paying' ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', transition: 'opacity 0.15s', marginBottom: '0.75rem' }}
        >
          {status === 'paying'
        ...
        <Link href={`/services/${service.id}`} style={{ display: 'block', textAlign: 'center', fontSize: '0.85rem', color: 'var(--ft-text-tertiary)', textDecoration: 'none', padding: '0.5rem' }}>
          ← Back to service
        </Link>
      </div>
    </div>
