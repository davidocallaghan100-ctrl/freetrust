...
const S: Record<string, React.CSSProperties> = {
  ...
  card: { background: 'var(--ft-surface)', border: '1px solid rgba(56,189,248,0.1)', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', textDecoration: 'none', color: 'inherit', transition: 'border-color 0.15s, transform 0.15s' },
        ...
        <div style={S.inner}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              ...
              <p style={{ color: 'var(--ft-text-tertiary)' }}>Discover and connect with impact-driven organisations on FreeTrust</p>
            </div>
            <Link href="/organisations/new" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: 'var(--ft-accent)', color: 'var(--ft-bg)', fontWeight: 700, fontSize: '0.88rem', padding: '0.6rem 1.25rem', borderRadius: 9, textDecoration: 'none', whiteSpace: 'nowrap' }}>
              🏢 Add Organisation
            </Link>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '1.25rem' }}>
            {TYPES.map(t => (
              <button key={t} onClick={() => setActiveType(t)} style={{
                ...
                color: activeType === t ? 'var(--ft-accent)' : 'var(--ft-text-secondary)',
              }}>{t}</button>
            ))}
          </div>
        </div>
      ...
      <div className="org-grid" style={S.grid}>
        {!loading && filtered.length === 0 && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '4rem 1rem' }}>
            ...
            <Link href="/organisations/new" style={{ display: 'inline-block', background: 'linear-gradient(135deg,var(--ft-accent),#0284c7)', color: '#fff', padding: '0.75rem 1.75rem', borderRadius: 10, fontWeight: 700, textDecoration: 'none' }}>
              Add Organisation
            </Link>
          </div>
        )}
            ...
            <div key={org.id} className="org-card" style={{ ...S.card, position: 'relative' }}>
              {canManage && (
                <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 5 }}>
                  ...
                  {openOptionsOrgId === org.id && (
                    <div className="org-options-menu" role="menu" style={{ position: 'absolute', top: 40, right: 0, width: 184, background: 'var(--ft-bg)', border: '1px solid rgba(148,163,184,0.22)', borderRadius: 12, padding: 6, boxShadow: '0 18px 44px rgba(0,0,0,0.45)' }}>
                      <Link href={`/organisations/${org.id}/edit`} role="menuitem" style={{ display: 'block', padding: '0.65rem 0.75rem', borderRadius: 8, color: 'var(--ft-text-secondary)', textDecoration: 'none', fontSize: '0.84rem', fontWeight: 600 }}>
                        ✏️ Edit organisation
                      </Link>
                      <button
                        type="button"
                        ...
                        style={{ width: '100%', textAlign: 'left', padding: '0.65rem 0.75rem', borderRadius: 8, border: 'none', background: 'transparent', color: 'var(--ft-danger)', fontSize: '0.84rem', fontWeight: 700, cursor: deletingOrgId === org.id ? 'wait' : 'pointer', fontFamily: 'inherit' }}
                      >
              ...
              <Link href={`/organisations/${org.id}`} style={{ display: 'flex', flexDirection: 'column', flex: 1, color: 'inherit', textDecoration: 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                  {org.logo_url ? (
                    <img src={org.logo_url} alt={name} style={{ width: 52, height: 52, borderRadius: 12, objectFit: 'cover', flexShrink: 0, background: 'var(--ft-bg)' }} />
                  ) : (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--ft-text)', lineHeight: 1.2, paddingRight: canManage ? 34 : 0 }}>{name}</span>
                      {org.is_verified && <span style={{ fontSize: '13px' }} title="Verified">✅</span>}
                    </div>
                    {org.type && <div style={{ fontSize: '0.75rem', color: 'var(--ft-accent)', marginTop: '0.2rem', fontWeight: 600 }}>{org.type}</div>}
                    {org.location && <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-tertiary)', marginTop: '0.1rem' }}>📍 {org.location}</div>}
                  </div>
                  {org.trust_score > 0 && (
                    <div style={{ background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.2)', borderRadius: 8, padding: '0.2rem 0.55rem', fontSize: '0.78rem', fontWeight: 700, color: 'var(--ft-accent)', flexShrink: 0, marginRight: canManage ? 36 : 0 }}>
                      ₮{org.trust_score}
                    </div>
                  )}
                </div>
                ...
                {(org.tags ?? []).length > 0 && (
                  <div style={{ display: 'flex', gap: '0.35rem', flexWrap: 'wrap' }}>
                    {(org.tags ?? []).slice(0, 4).map(tag => (
                      <span key={tag} style={{ background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.12)', borderRadius: 999, padding: '0.15rem 0.55rem', fontSize: '0.72rem', color: 'var(--ft-text-secondary)' }}>{tag}</span>
                    ))}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(56,189,248,0.06)', paddingTop: '0.75rem', marginTop: 'auto' }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--ft-text-faint)' }}>
