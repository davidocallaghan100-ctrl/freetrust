  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 58px)', background: 'var(--ft-bg)', color: 'var(--ft-text)', fontFamily: 'system-ui', paddingTop: 64 }}>
      <style>{`
        ...
        .photo-thumb { position: relative; border-radius: 10px; overflow: hidden; aspect-ratio: 1; background: var(--ft-surface); border: 2px solid var(--ft-border-strong); cursor: default; }
        .photo-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .photo-thumb .rm-btn { position: absolute; top: 4px; right: 4px; background: rgba(15,23,42,0.85); border: 1px solid var(--ft-border-strong); border-radius: 50%; width: 22px; height: 22px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--ft-danger); font-size: 11px; font-weight: 700; backdrop-filter: blur(4px); }
        ...
        .photo-thumb .move-btn { background: rgba(15,23,42,0.85); border: 1px solid var(--ft-border-strong); border-radius: 5px; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--ft-text-secondary); font-size: 10px; }
        .photo-thumb .move-btn:hover:not(:disabled) { color: var(--ft-accent); border-color: rgba(56,189,248,0.4); }
        .photo-thumb .move-btn:disabled { opacity: 0.3; cursor: default; }
        .photo-thumb .order-badge { position: absolute; top: 4px; left: 4px; background: rgba(15,23,42,0.85); border: 1px solid var(--ft-border-strong); border-radius: 4px; padding: 1px 5px; font-size: 10px; font-weight: 700; color: var(--ft-text-tertiary); }
        .photo-thumb:first-child .order-badge { color: var(--ft-accent); border-color: rgba(56,189,248,0.3); }
        ...
        .rs-cover-hint { font-size: 0.72rem; color: var(--ft-text-faint); }
      ...
      <div className="rs-edit-wrap">
        ...
        <div style={{ marginBottom: '2rem' }}>
          <Link href={`/rent-share/${id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--ft-text-tertiary)', textDecoration: 'none', fontSize: '0.85rem', marginBottom: '1rem' }}>
            ← Back to listing
          </Link>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 800, margin: '0 0 0.4rem' }}>Edit Listing</h1>
          <p style={{ color: 'var(--ft-text-tertiary)', margin: 0 }}>Update your item details and photos</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {CATEGORIES.map(c => (
                <button key={c} type="button" onClick={() => setCategory(c)} style={{
                  ...
                  background: category === c ? 'rgba(45,212,191,0.15)' : 'var(--ft-surface)',
                  border: category === c ? '1px solid rgba(45,212,191,0.4)' : '1px solid var(--ft-border-strong)',
                  color: category === c ? '#2dd4bf' : 'var(--ft-text-secondary)',
                }}>{c}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              {[
                ...
                { value: 'inactive', label: '✕ Inactive',   color: 'var(--ft-text-secondary)', bg: 'rgba(148,163,184,0.12)' },
              ].map(s => (
                <button key={s.value} type="button" onClick={() => setStatus(s.value)} style={{
                  ...
                  background: status === s.value ? s.bg : 'var(--ft-surface)',
                  border: status === s.value ? `1px solid ${s.color}50` : '1px solid var(--ft-border-strong)',
                  color: status === s.value ? s.color : 'var(--ft-text-tertiary)',
                }}>{s.label}</button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="rs-pricing-grid">
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-faint)', marginBottom: 5 }}>Per Day</div>
              ...
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-faint)', marginBottom: 5 }}>Per Week</div>
          ...
          <div>
            ...
            <div className="rs-avail-grid">
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-faint)', marginBottom: 5 }}>Available From</div>
                <input type="date" value={availableFrom} onChange={e => setAvailableFrom(e.target.value)} style={{ ...inputStyle, colorScheme: 'dark' }} />
              </div>
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--ft-text-faint)', marginBottom: 5 }}>Available To</div>
          ...
          {/* Error */}
          {error && (
            <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 10, padding: '10px 14px', color: 'var(--ft-danger)', fontSize: '0.875rem' }}>
              {error}
            </div>
          )}
          ...
          <div className="rs-edit-actions">
            <Link href={`/rent-share/${id}`} style={{
              flex: '0 0 auto', background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: 12,
              padding: '12px 20px', fontSize: 14, fontWeight: 600, color: 'var(--ft-text-secondary)',
            ...
            <button
              ...
              disabled={loading}
              style={{
                flex: 1, background: loading ? 'var(--ft-surface)' : 'linear-gradient(135deg,#2dd4bf,#0891b2)',
                border: 'none', borderRadius: 12, padding: '12px 0', fontSize: 15, fontWeight: 700,
                color: loading ? 'var(--ft-text-faint)' : 'var(--ft-bg)', cursor: loading ? 'not-allowed' : 'pointer',
                fontFamily: 'inherit',
              }}
            >
              {savingLabel}
