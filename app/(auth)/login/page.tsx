...
function LoginForm() {
  ...
  const handleOAuthLogin = async (provider: OAuthProvider, queryParams?: Record<string, string>) => {
    ...
    try {
      ...
      console.error(`[login] ${provider} OAuth threw:`, err)
                ...
                {VISIBLE_OAUTH_PROVIDERS.map(({ provider, label, icon, queryParams }) => {
                  ...
                  return (
                    <button
                      ...
                      className="btn-oauth"
                      type="button"
                      onClick={() => handleOAuthLogin(provider, queryParams)}
