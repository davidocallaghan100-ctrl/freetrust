...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontSize: '14px', color: 'var(--ft-text-tertiary)' }}>Loading…</div>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', color: 'var(--ft-text)', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div className="dash-grid">
        ...
        {/* ── Profile card ── */}
        <div style={{ background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: '16px', padding: '1.25rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <Avatar url={profile?.avatar_url ?? null} name={profile?.full_name} size={60} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: '18px', fontWeight: 800, color: 'var(--ft-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</div>
            {profile?.username && <div style={{ fontSize: '13px', color: 'var(--ft-text-tertiary)' }}>@{profile.username}</div>}
            {profile?.location && <div style={{ fontSize: '12px', color: 'var(--ft-text-faint)', marginTop: '2px' }}>📍 {profile.location}</div>}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0 }}>
            <div style={{ background: 'rgba(56,189,248,0.1)', border: '1px solid rgba(56,189,248,0.25)', borderRadius: '10px', padding: '6px 14px', textAlign: 'center' }}>
              <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--ft-accent)' }}>₮{trust.toFixed(0)}</div>
              <div style={{ fontSize: '10px', color: 'var(--ft-text-tertiary)', fontWeight: 600 }}>TRUST</div>
            </div>
            <Link href="/profile" style={{ fontSize: '12px', color: 'var(--ft-accent)', textDecoration: 'none', fontWeight: 600 }}>Edit profile →</Link>
          </div>
        </div>
        ...
        <div className="stat-row">
          {[
            ...
            <Link key={s.label} href={s.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: '12px', padding: '0.85rem', textAlign: 'center', cursor: 'pointer' }}>
                <div style={{ fontSize: '22px', marginBottom: '4px' }}>{s.icon}</div>
                <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--ft-text)' }}>{s.value}</div>
                <div style={{ fontSize: '11px', color: 'var(--ft-text-tertiary)', fontWeight: 600 }}>{s.label}</div>
              </div>
            </Link>
          ))}
        </div>
        ...
        {/* ── Quick actions ── */}
        <div style={{ fontSize: '11px', fontWeight: 700, color: 'var(--ft-text-faint)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.65rem' }}>Quick access</div>
        <div className="quick-grid" style={{ marginBottom: '1.5rem' }}>
          {QUICK_LINKS.map(({ href, icon, label }) => (
            <Link key={href} href={href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: '10px', padding: '0.75rem 0.5rem', textAlign: 'center', cursor: 'pointer' }}>
                <div style={{ fontSize: '20px', marginBottom: '4px' }}>{icon}</div>
                <div style={{ fontSize: '11px', color: 'var(--ft-text-secondary)', fontWeight: 500 }}>{label}</div>
              </div>
            </Link>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
          <Link href="/create" style={{ textDecoration: 'none' }}>
            <div style={{ background: 'linear-gradient(135deg, rgba(56,189,248,0.12), rgba(56,189,248,0.04))', border: '1px solid rgba(56,189,248,0.25)', borderRadius: '12px', padding: '1rem', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ft-text)' }}>Create a post</div>
              <div style={{ fontSize: '11px', color: 'var(--ft-text-tertiary)', marginTop: '2px' }}>Share with the community</div>
            </div>
          </Link>
          <Link href="/seller/gigs/create" style={{ textDecoration: 'none' }}>
            <div style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.1), rgba(52,211,153,0.03))', border: '1px solid rgba(52,211,153,0.2)', borderRadius: '12px', padding: '1rem', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--ft-text)' }}>List a service</div>
              <div style={{ fontSize: '11px', color: 'var(--ft-text-tertiary)', marginTop: '2px' }}>Start earning Trust</div>
            </div>
          </Link>
        </div>
