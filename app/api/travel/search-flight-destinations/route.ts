...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = new URL(`https://${host}/api/v1/flights/searchDestination`)
    ...
    if (!res.ok) return failure(data?.message || data?.error || 'Travel partner flight destination search failed', res.status)

    const destinations = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []
    return NextResponse.json({ destinations })
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Unexpected flight destination search error'
