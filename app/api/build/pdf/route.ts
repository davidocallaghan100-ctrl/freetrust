...
export async function POST(req: NextRequest) {
  try {
    ...
    const drawHeading = (text: string, size = 16) => {
      ensureSpace(size + 14)
      page.drawText(text, { x: MARGIN, y, size, font: bold, color: teal })
      y -= size + 10
    }
    ...
    // Title
    page.drawText('FreeTrust — Build', { x: MARGIN, y, size: 20, font: bold, color: teal })
    y -= 28
    page.drawText(conversation.title || latestSpec?.name || 'Untitled design', { x: MARGIN, y, size: 15, font: bold, color: dark })
    y -= 20
    ...
    const disclaimerLines = wrapText(DISCLAIMER_TEXT, font, 8.5, CONTENT_W - 16)
    let dy = y - 12
    ...
    for (const meta of BUILD_SECTIONS) {
      ...
      if (!content) continue
      drawHeading(`${meta.icon} ${meta.label}`, 14)
