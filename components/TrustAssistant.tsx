...
// ─── Page context hints ───────────────────────────────────────────────────────
const PAGE_HINT_KEYS: Array<[string, string]> = [
  ['/services', 'services'],
  ['/products', 'products'],
  ['/wallet', 'wallet'],
  ['/create', 'create'],
...
function getPageHintKey(path: string): string | null {
  for (const [prefix, key] of PAGE_HINT_KEYS) {
...
export default function TrustAssistant() {
  ...
  const t = useTranslations('trustAssistant')
  ...
  useEffect(() => {
    ...
    const hint = hintKey ? t(`hints.${hintKey}`) : null
  ...
  }, [pathname, t]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleOpen = useCallback(() => {
    ...
    if (messages.length === 0) {
      ...
      const hint = hintKey ? t(`hints.${hintKey}`) : null
      const greeting = user
        ? t('greeting.user', { name: firstName, balance: user.trustBalance, hint: hint ? `${hint}\n\n` : '' })
        : t('greeting.guest', { hint: hint ? `${hint}\n\n` : '' })
  ...
  const send = useCallback(async (text?: string) => {
    ...
    try {
      ...
      const assistantMsg: Message = {
        ...
        role: 'assistant',
        content: data.reply ?? t('errors.moment'),
        ts: Date.now(),
  ...
  }, [input, loading, conversationId, pathname, messages, user, t])
  ...
  const escalate = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        content: t('escalation.created'),
      }])
    } catch {
      setMessages(prev => [...prev, { id: crypto.randomUUID(), role: 'assistant', ts: Date.now(), content: t('escalation.failed') }])
    } finally {
      setEscalating(false)
    }
  }
  ...
  const quickReplies: string[] = (() => {
    if (pathname.startsWith('/wallet')) return t.raw('quickReplies.wallet') as string[]
    if (pathname.startsWith('/services') || pathname.startsWith('/products')) return t.raw('quickReplies.marketplace') as string[]
    if (pathname.startsWith('/profile')) return t.raw('quickReplies.profile') as string[]
    if (pathname.startsWith('/community')) return t.raw('quickReplies.community') as string[]
    if (pathname.startsWith('/orders') || pathname.startsWith('/disputes')) return t.raw('quickReplies.orders') as string[]
    if (pathname.startsWith('/jobs')) return t.raw('quickReplies.jobs') as string[]
    return t.raw('quickReplies.default') as string[]
  })()
          ...
          {messages.length >= 4 && !escalated && (
            <div style={{ padding: '4px 14px', flexShrink: 0 }}>
              <button className="ta-escalate" onClick={escalate} disabled={escalating}
                ...
                {escalating ? t('escalation.creating') : t('escalation.talkToHuman')}
              </button>
            </div>
          )}
