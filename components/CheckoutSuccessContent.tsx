...
export default function CheckoutSuccessContent() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 40, height: 40, borderRadius: '50%', border: '3px solid var(--ft-surface)', borderTopColor: 'var(--ft-accent)', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
          <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
          <div style={{ fontSize: 13, color: 'var(--ft-text-tertiary)' }}>Loading order details…</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--ft-bg)', color: 'var(--ft-text)', fontFamily: 'system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}>
      <style>{`
      ...
      <div style={{ width: '100%', maxWidth: 440, animation: 'fadeUp 0.4s ease' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--ft-text-tertiary)', lineHeight: 1.6, margin: 0, maxWidth: 320, marginInline: 'auto' }}>
        ...
        <div style={{ background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--ft-text-tertiary)', marginTop: 2 }}>Funds are protected until you confirm receipt</div>
          </div>
        </div>

        {/* Order summary card */}
        <div style={{ background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: 16, overflow: 'hidden', marginBottom: 20 }}>
          <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--ft-border-strong)' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ft-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Order Summary</div>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[80, 60, 70, 50].map((w, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ height: 12, width: `${w}%`, background: 'linear-gradient(90deg,var(--ft-surface) 25%,#243047 50%,var(--ft-surface) 75%)', backgroundSize: '200%', animation: 'shimmer 1.5s infinite', borderRadius: 6 }} />
                </div>
              ))}
            ...
            <div style={{ padding: '4px 0' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: '1px solid rgba(51,65,85,0.6)' }}>
                <span style={{ fontSize: 13, color: 'var(--ft-text-secondary)' }}>Item</span>
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ft-text)', maxWidth: 220, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{summary.itemName}</span>
              </div>
              {/* Total paid */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: '1px solid rgba(51,65,85,0.6)' }}>
                <span style={{ fontSize: 13, color: 'var(--ft-text-secondary)' }}>Total Paid</span>
                <span style={{ fontSize: 16, fontWeight: 800, color: 'var(--ft-text)' }}>{amountDisplay}</span>
              </div>
              {/* Platform fee */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px', borderBottom: '1px solid rgba(51,65,85,0.6)' }}>
                <span style={{ fontSize: 12, color: 'var(--ft-text-tertiary)' }}>Platform fee ({feeLabel})</span>
                <span style={{ fontSize: 13, color: 'var(--ft-text-tertiary)' }}>−{feeDisplay}</span>
              </div>
              {/* Seller receives */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 18px' }}>
                <span style={{ fontSize: 13, color: 'var(--ft-text-secondary)' }}>Seller receives</span>
            ...
            <div style={{ padding: '20px 18px', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: 'var(--ft-text-tertiary)', lineHeight: 1.6 }}>
        ...
        <div style={{ background: 'rgba(56,189,248,0.07)', border: '1px solid rgba(56,189,248,0.18)', borderRadius: 12, padding: '12px 16px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ft-accent)' }}>+₮25 Trust earned</div>
            <div style={{ fontSize: 12, color: 'var(--ft-text-tertiary)', marginTop: 2 }}>Completing orders grows your Trust score</div>
          </div>
        </div>

        {/* Action buttons */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <Link href="/orders" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'linear-gradient(135deg,var(--ft-accent),#0284c7)', color: 'var(--ft-bg)', borderRadius: 12, padding: '14px 20px', fontWeight: 800, fontSize: 15, textDecoration: 'none', textAlign: 'center' }}>
            📦 View My Orders
          </Link>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <Link href="/services" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(56,189,248,0.08)', border: '1px solid rgba(56,189,248,0.2)', color: 'var(--ft-accent)', borderRadius: 12, padding: '12px', fontWeight: 700, fontSize: 13, textDecoration: 'none', textAlign: 'center' }}>
              🛠 Services
            </Link>
            <Link href="/" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.15)', color: 'var(--ft-text-secondary)', borderRadius: 12, padding: '12px', fontWeight: 700, fontSize: 13, textDecoration: 'none', textAlign: 'center' }}>
              🏠 Home
            </Link>
          </div>
        </div>
        ...
        {/* Trust footer */}
        <div style={{ textAlign: 'center', marginTop: 20, fontSize: 12, color: 'var(--ft-border-strong)' }}>
