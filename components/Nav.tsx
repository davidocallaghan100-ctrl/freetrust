  ...
  return (
    <>
      ...
      <header style={{
        ...
        background: 'var(--ft-bg)', borderBottom: '1px solid var(--ft-border)',
        ...
        {!loading && user && (
          <div ref={profileRef} style={{ position: 'relative', flexShrink: 0 }}>
            ...
            {profileOpen && (
              <div style={{ position: 'fixed', left: '12px', top: '62px', background: 'var(--ft-surface)', border: '1px solid var(--ft-border-strong)', borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.5)', minWidth: '220px', maxWidth: 'calc(100vw - 24px)', overflow: 'hidden', zIndex: 200 }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--ft-border-strong)', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <Avatar url={activeProfileIdentity.image} name={activeProfileIdentity.name} email={user.email} size={38} />
                  <div style={{ overflow: 'hidden', flex: 1 }}>
                    <div style={{ fontSize: '13px', color: 'var(--ft-text)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{activeProfileIdentity.name}</div>
                    <Link href={feedIdentity?.type === 'org' ? pageHref(feedIdentity) : '/profile'} onClick={() => setProfileOpen(false)} style={{ fontSize: '11px', color: feedIdentity?.type === 'org' ? '#86efac' : 'var(--ft-text-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block', textDecoration: 'none' }}>{activeProfileIdentity.subtitle}</Link>
                  </div>
                </div>
                <div style={{ padding: '8px 8px 4px' }}>
                  <div style={{ color: 'var(--ft-text-secondary)', fontSize: 10, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', padding: '2px 8px 7px' }}>
                    {tProfile('switchProfile')}
                  </div>
                  <button type="button" onClick={() => chooseFeedIdentity('/profile', { type: 'personal', id: user.id, name: user.name ?? tProfile('myProfile'), username: null, avatar_url: user.avatar })} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '8px', fontSize: '13px', color: 'var(--ft-text)', textDecoration: 'none', borderRadius: 10, background: (pathname.startsWith('/profile') || feedIdentity?.type === 'personal') ? 'rgba(56,189,248,0.12)' : 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                    ...
                    <span style={{ minWidth: 0, flex: 1 }}>
                      ...
                      <span style={{ display: 'block', color: 'var(--ft-text-secondary)', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tProfile('personalProfile')}</span>
                    </span>
                    {feedIdentity?.type === 'personal' ? <span style={{ color: 'var(--ft-accent)', fontSize: 14 }}>✓</span> : null}
                  </button>
                  {pagesLoading ? (
                    <div style={{ color: 'var(--ft-text-secondary)', fontSize: 12, padding: '8px' }}>{tProfile('loadingPages')}</div>
                  ) : adminPages.length === 0 ? (
                    <div style={{ color: 'var(--ft-text-tertiary)', fontSize: 12, padding: '8px' }}>{tProfile('noAdminPages')}</div>
                  ) : adminPages.map(page => (
                    <button key={page.id} type="button" onClick={() => chooseFeedIdentity(pageHref(page), { type: 'org', ...page })} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '8px', fontSize: '13px', color: 'var(--ft-text)', textDecoration: 'none', borderRadius: 10, background: feedIdentity?.type === 'org' && feedIdentity.id === page.id ? 'rgba(34,197,94,0.12)' : 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
                ...
                <div style={{ height: 1, background: 'var(--ft-border-strong)' }} />
                 {[
                   ...
                   <Link key={href} href={href} onClick={() => setProfileOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 16px', fontSize: '13px', color: 'var(--ft-text-secondary)', textDecoration: 'none' }}>
                     <span style={{ ...EMOJI_STYLE, fontSize: '14px' }}>{icon}</span>
                     <span>{label}</span>
                   </Link>
                 ))}
                <div style={{ borderTop: '1px solid var(--ft-border-strong)' }}>
                  <button onClick={handleSignOut} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', fontSize: '13px', color: 'var(--ft-danger)', background: 'none', border: 'none', cursor: 'pointer' }}>
                    {tAuth('signOut')}
                  </button>
                </div>
              </div>
        ...
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none', flexShrink: 0 }}>
          ...
          <span style={{ color: 'var(--ft-text)', fontWeight: 700, fontSize: '15px', letterSpacing: '-0.3px' }}>FreeTrust</span>
        </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, paddingRight: '8px' }}>
          ...
          {loading && <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--ft-surface)', flexShrink: 0 }} />}
          ...
          >
            <span style={{ display: 'block', width: '20px', height: '2px', background: 'var(--ft-text-secondary)', borderRadius: '2px', transition: 'all 0.25s ease', transform: drawerOpen ? 'translateY(7px) rotate(45deg)' : 'none' }} />
            <span style={{ display: 'block', width: '20px', height: '2px', background: 'var(--ft-text-secondary)', borderRadius: '2px', transition: 'all 0.25s ease', opacity: drawerOpen ? 0 : 1, transform: drawerOpen ? 'scaleX(0)' : 'none' }} />
            <span style={{ display: 'block', width: '20px', height: '2px', background: 'var(--ft-text-secondary)', borderRadius: '2px', transition: 'all 0.25s ease', transform: drawerOpen ? 'translateY(-7px) rotate(-45deg)' : 'none' }} />
          </button>
        </div>
      </header>
      ...
      <nav style={{
        ...
        {/* Drawer header */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--ft-border)', height: '58px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src={FREETRUST_LOGO_SRC} alt="FreeTrust" style={{ ...FREETRUST_LOGO_STYLE, width: '28px', height: '28px' }} />
            <span style={{ color: 'var(--ft-text)', fontWeight: 700, fontSize: '15px', letterSpacing: '-0.3px' }}>FreeTrust</span>
          </div>
          <button onClick={() => setDrawerOpen(false)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', background: 'var(--ft-surface)', border: 'none', borderRadius: '8px', cursor: 'pointer', color: 'var(--ft-text-secondary)', fontSize: '16px' }} aria-label={tNav('closeMenu')}>
            ✕
          </button>
        </div>
        ...
        {user && (
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--ft-border)', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ overflow: 'hidden', flex: 1 }}>
              <div style={{ fontSize: '14px', color: 'var(--ft-text)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.name ?? tProfile('yourProfile')}</div>
              <Link href="/profile" onClick={() => setDrawerOpen(false)} style={{ fontSize: '12px', color: 'var(--ft-text-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block', textDecoration: 'none' }}>{user.email}</Link>
            </div>
          </div>
        )}
        ...
        <div style={{ padding: '12px 0', flex: 1 }}>
          {visibleDrawerSections.map(section => (
            <div key={section.label} style={{ marginBottom: '8px' }}>
              <div style={{ padding: '8px 20px 4px', fontSize: '10px', fontWeight: 700, color: 'var(--ft-text-faint)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                ...
                return (
                  <Link
                    ...
                    style={{
                      ...
                      borderLeft: active ? '3px solid var(--ft-accent)' : '3px solid transparent',
        ...
        <div style={{ borderTop: '1px solid var(--ft-border)', paddingTop: '8px', paddingBottom: '16px' }}>
          <div style={{ padding: '8px 20px 4px', fontSize: '10px', fontWeight: 700, color: 'var(--ft-text-faint)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            {tNav('account')}
          </div>
          <button
            ...
            style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', padding: '10px 20px',
              width: '100%', textAlign: 'left', fontSize: '14px', fontWeight: 400,
              color: 'var(--ft-text-secondary)', background: 'none', border: 'none',
              borderLeft: '3px solid transparent', cursor: 'pointer', transition: 'all 0.15s ease',
              fontFamily: 'inherit',
            }}
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ ...EMOJI_STYLE, fontSize: '16px' }}>{theme === 'dark' ? '🌙' : '☀️'}</span>
            ...
            <span
              ...
              style={{
                ...
                background: theme === 'light' ? 'var(--ft-accent)' : 'var(--ft-border-strong)',
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  color: active ? 'var(--ft-accent)' : 'var(--ft-text-secondary)', textDecoration: 'none',
                  background: active ? 'rgba(56,189,248,0.08)' : 'transparent',
                  borderLeft: active ? '3px solid var(--ft-accent)' : '3px solid transparent',
                  transition: 'all 0.15s ease',
                }}
          ...
          {user ? (
            <button
              ...
              style={{
                display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 20px',
                width: '100%', textAlign: 'left', fontSize: '14px', color: 'var(--ft-danger)',
            ...
            <>
              <Link href="/login" style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 20px', fontSize: '14px', color: 'var(--ft-accent)', textDecoration: 'none', borderLeft: '3px solid transparent' }}>
                <span style={{ fontSize: '16px', lineHeight: 1 }}>🔑</span>
                {tAuth('signIn')}
              </Link>
